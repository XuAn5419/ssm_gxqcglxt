...
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>预约出租管理 <span class="c-gray en">&gt;</span> 预约出租列表 <a
	...
	<div class="page-container">
		...
		<form action="orders/deleteOrdersByIds.action" method="post" id="myform" name="myform">
			<table class="table table-border table-bordered table-bg">
				<thead>
					<tr>
						<th scope="col" colspan="11">预约出租列表</th>
					</tr>
					<tr class="text-c">
				...
				<c:forEach items="${ordersList}" var="orders">
					<tr class="text-c">
						...
						<td>${orders.status}</td>
						<td>${orders.memo}</td>
						<td><c:if test="${orders.status eq '待审核' }">
								<a href="orders/status.action?id=${orders.ordersid}">通过审核</a>&nbsp;&nbsp;
								<a href="orders/refuse.action?id=${orders.ordersid}">拒绝预约</a>&nbsp;&nbsp;
								</c:if><a href="orders/deleteOrders.action?id=${orders.ordersid}" onclick="{if(confirm('确定要删除吗?')){return true;}return false;}" title="删除"><i
