...
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>用户车辆管理 <span class="c-gray en">&gt;</span> 用户车辆查询 <a
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="10">用户车辆查询</th>
				</tr>
				<tr class="text-c">
					<th class="center">车主</th>
			...
			<c:forEach items="${pcarsList}" var="pcars">
				<tr class="text-c">
					<td class="center">${pcars.username}</td>
					<td class="center">${pcars.carsno}</td>
					<td class="center">${pcars.catename}</td>
					<td class="center">${pcars.workdate}</td>
					<td class="center">${pcars.youhao}</td>
					<td class="center">${pcars.pailiang}</td>
					<td class="center">${pcars.color}</td>
				</tr>
			</c:forEach>
		</table>
		<div class="cl pd-5 mt-20"></div>
		<div class="text-c">
			<form action="pcars/queryPcarsByCond.action" name="myform" method="post">
				查询条件： <span class="select-box" style="width: 200px;"><select name="cond" class="select">
						<option value="usersid">按车主查询</option>
						...
						<option value="color">按车身颜色查询</option>
