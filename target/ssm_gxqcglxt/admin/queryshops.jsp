...
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>网点管理 <span class="c-gray en">&gt;</span> 网点查询 <a
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="7">网点查询</th>
				</tr>
				<tr class="text-c">
					<th class="center">网点名称</th>
			...
			<c:forEach items="${shopsList}" var="shops">
				<tr class="text-c">
					<td class="center">${shops.shopsname}</td>
					<td class="center">${shops.manager}</td>
					<td class="center">${shops.addtime}</td>
					<td class="center">${shops.contact}</td>
					<td class="center">${shops.address}</td>
					<td class="center">${shops.memo}</td>
				</tr>
			</c:forEach>
		</table>
		<div class="cl pd-5 mt-20"></div>
		<div class="text-c">
			<form action="shops/queryShopsByCond.action" name="myform" method="post">
				查询条件： <span class="select-box" style="width: 200px;"><select name="cond" class="select">
						<option value="shopsname">按网点名称查询</option>
